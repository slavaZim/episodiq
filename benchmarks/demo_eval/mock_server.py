...
_DATASETS = {
    "swe-smith": (
        ...
        "instance_id", "traj_id", "messages", True,  # repo-filter is substring on instance_id; messages is JSON string
    ),
    "nebius": (
        "nebius/SWE-rebench-openhands-trajectories", "train",
        "repo", "trajectory_id", "trajectory", False,  # repo-filter is exact match on `repo`; trajectory is list[dict]
    ),
}


def _load_dataset(repo_filter: str = "getmoto__moto", dataset: str = "swe-smith") -> int:
    """Load trajectories from HF and build assistant-response index."""
    from datasets import load_dataset

    if dataset not in _DATASETS:
        raise ValueError(f"Unknown dataset: {dataset}")
    hf_id, split, repo_field, traj_id_field, msgs_field, msgs_json = _DATASETS[dataset]
    logger.info("Loading %s split=%s ...", hf_id, split)
    ds = load_dataset(hf_id, split=split)
    logger.info("Loaded %d trajectories total", len(ds))

    if dataset == "swe-smith":
        rows = [r for r in ds if repo_filter in r[repo_field]]
    else:
        rows = [r for r in ds if r.get(repo_field) == repo_filter]
    logger.info("Filtered to %d trajectories for repo=%s", len(rows), repo_filter)

    for row in rows:
        traj_id = row[traj_id_field]
        _known_traj_ids.add(traj_id)

        raw = row[msgs_field]
        messages = json.loads(raw) if (msgs_json and isinstance(raw, str)) else raw
        turn = 0
        for msg in messages:
...
def main():
    ...
    parser.add_argument("--dataset", default="swe-smith", choices=list(_DATASETS))
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")

    n = _load_dataset(repo_filter=args.repo_filter, dataset=args.dataset)
    if n == 0:
