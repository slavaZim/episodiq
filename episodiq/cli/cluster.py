...
from episodiq.clustering.tokenizer.grid_search import (
    GridSearchReport as TokenizerGridSearchReport,
    TokenizerGridSearch,
)
...
def build_paths(
    ...
    Paths are rebuilt without profiles; run `train-profile` afterwards to
    backfill the VLAD `profile` column.
    """
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
...
def tokenize(
    ...
    umap_dims: Optional[int] = typer.Option(None, "--umap-dims", "-d", help="UMAP dimensions"),
    umap_n_neighbors: Optional[int] = typer.Option(None, "--umap-nn", help="UMAP n_neighbors"),
    selection_method: Optional[str] = typer.Option(None, "--selection-method", help="HDBSCAN cluster_selection_method"),
    selection_epsilon: Optional[float] = typer.Option(None, "--selection-epsilon", help="HDBSCAN cluster_selection_epsilon"),
    dry_run: bool = typer.Option(False, "--dry-run", help="Skip DB writes"),
) -> None:
    """Cluster (action, observation) act_obs embeddings into act_obs-level tokens.

    Rebuilds token_clusters + token_mapping from the current cluster centroids
    on all completed trajectory_paths. Run after `cluster run` + `build-paths`.
    """
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    ...
    params = (
        Params(**{**TOKENIZER_DEFAULT_PARAMS.__dict__, **overrides})
        if overrides else TOKENIZER_DEFAULT_PARAMS
    )
    ...
    console.print(
        f"tokenize: n_clusters={result.n_clusters} noise={result.noise_count} "
        f"dbcv={result.dbcv:.3f} entropy={result.entropy:.3f}"
    )


@app.command(name="tokenize-grid")
def tokenize_grid(
    env: Path = typer.Option(Path(".env"), "--env", help="Path to .env file"),
    ...
    selection_method: Optional[str] = typer.Option(None, "--selection-method", help="Comma-separated methods"),
    selection_epsilon: Optional[str] = typer.Option(None, "--selection-epsilon", help="Comma-separated epsilon"),
    save_output: Optional[Path] = typer.Option(None, "--save-output", help="Save full report as CSV"),
) -> None:
    """Grid-search UMAP+HDBSCAN params for the AO tokenizer (no DB writes)."""
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")
    ...
    async def _run():
        return await TokenizerGridSearch(session_factory, params_list=custom_grid).run()

    report = asyncio.run(_run())
    console.print(_tokenizer_grid_table(report))
    if save_output:
        _save_tokenizer_csv(report, save_output)


def _tokenizer_grid_table(report: TokenizerGridSearchReport, top_n: int = 10) -> Table:
    if not report.entries:
        return Table(title="Tokenizer grid (no results)")
    winner = report.entries[0]
    title = (
        f"Tokenizer grid (winner: min_cs={winner.params.min_cluster_size}, "
        f"min_s={winner.params.min_samples}, umap_d={winner.params.umap_dims})"
    )
    ...
    table.add_column("Entropy", justify="right")
    table.add_column("Score", justify="right", style="bold")
    for i, e in enumerate(report.entries[:top_n], 1):
        table.add_row(
            str(i),
            str(e.params.min_cluster_size),
            str(e.params.min_samples),
...
def _save_tokenizer_csv(report: TokenizerGridSearchReport, path: Path) -> None:
    fieldnames = [
        "min_cluster_size", "min_samples", "umap_dims", "umap_n_neighbors",
        "cluster_selection_method", "cluster_selection_epsilon",
        "n_clusters", "noise_count", "noise_ratio", "dbcv", "entropy", "score",
    ]
    with open(path, "w", newline="") as f:
    ...
    console.print(f"Saved tokenizer grid report to {path}")
