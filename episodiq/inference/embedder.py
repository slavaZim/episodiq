...
class EmbedderClient:
    ...
    async def startup(self) -> None:
        ...
        self._client = httpx.AsyncClient(
            ...
            limits=httpx.Limits(
                ...
                max_keepalive_connections=max(1, self.config.max_connections // 2),
            ),
        )
    ...
    async def embed(self, texts: list[str], dims: int) -> list[list[float]]:
        ...
        constant_backoff = self.config.backoff_mode == "constant"
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    raise
                delay = retry_backoff if constant_backoff else retry_backoff * (2 ** attempt)
