...
class RetrievalSweep:
    ...
    def _score_combo(
        ...
        auc = self._current_auc_at_step(per_traj, self._min_step, traj_status)
        cov = self._coverage_at_step(all_query_snaps, per_traj, self._min_step)
        return SweepPoint(
            top_k=top_k,
