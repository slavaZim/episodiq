...
class TestClusteringAnnotationIntegration:
    ...
    async def test_clustering_then_annotation_merge(self, session_factory):
        ...
        async with session_factory() as session:
            # Index 0 is an initial-task observation that the
            # skip_initial_observation filter excludes from clustering. It
            # exists so every group observation has an earlier predecessor
            # and survives the filter.
            session.add(Message(
                trajectory_id=trajectory_id, role="user",
                content=[{"type": "text", "text": "initial task"}],
                embedding=l2_normalize(np.random.RandomState(7).randn(DIM).tolist()),
                category="text", cluster_type="observation", index=0,
            ))
            idx = 1
        ...
        async with session_factory() as session:
            ...
            # Task obs (index 0) is excluded by skip_initial_observation; the
            # 15 group observations should all be clustered.
            clustered = [m for m in msgs if m.cluster_id is not None]
