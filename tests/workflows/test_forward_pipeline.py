...
class TestLoggingPipelineIntegration:
    ...
    @respx.mock
    async def test_build_path_assigns_ao_token(self, session_factory, echo_adapter):
        """Closing the prev path online resolves (action_cluster,
        to_observation_cluster) → ordinal via TokenAssigner and writes
        trace_tokens on the closed row; the new pending row inherits it.
        """
        target_ordinal = 42
        embedder = MockEmbedder(embedding_fn=text_to_embedding, dims=1024)
        manager = TrajectoryManager(postprocess_timeout=10.0)
        ...
        # Seed clusters + one member per cluster so ClusterAssigner.find_neighbors
        # finds them when live messages arrive, and seed a token_mapping row so
        # TokenAssigner.assign hits a direct match for the closing transition.
        async with session_factory() as session:
            obs_cluster = Cluster(
                type="observation", category="text", label="o:text:answer",
            )
            act_cluster = Cluster(
                type="action", category="text", label="a:text:greet",
            )
            session.add_all([obs_cluster, act_cluster])
            await session.flush()
            seed_traj = uuid4()
            session.add(Trajectory(id=seed_traj))
            await session.flush()
            session.add(Message(
                trajectory_id=seed_traj, role="user", content=[],
                embedding=text_to_embedding("what is 2+2?", 1024),
                category="text", cluster_type="observation",
                cluster_id=obs_cluster.id, index=0,
            ))
            session.add(Message(
                trajectory_id=seed_traj, role="assistant", content=[],
                embedding=text_to_embedding("hi there", 1024),
                category="text", cluster_type="action",
                cluster_id=act_cluster.id, index=1,
            ))
            tc = TokenCluster(cluster_id=target_ordinal, centroid=[0.0] * 2048)
            session.add(tc)
            await session.flush()
            session.add(TokenMapping(
                action_label=act_cluster.label,
                observation_label=obs_cluster.label,
                action_cluster_id=act_cluster.id,
                observation_cluster_id=obs_cluster.id,
        ...
        async with session_factory() as session:
            ...
            paths = await path_repo.find_by(trajectory_id=trajectory_id)
        assert len(paths) == 2
        p0, p1 = sorted(paths, key=lambda p: p.index)
        assert p0.trace_tokens == [target_ordinal]
        assert p1.trace_tokens == [target_ordinal]
