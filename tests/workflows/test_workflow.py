...
class TestWorkflow:
    ...
    async def test_deadline_resets_after_untimed_step(
        self, mock_adapter, mock_embedder, mock_request, monkeypatch,
    ):
        """A slow untimed step (e.g. forward) doesn't eat the budget of
        sync steps that come after it.
        """
        mock_cfg = MagicMock()
        mock_cfg.process_input_timeout = 0.05
        monkeypatch.setattr("episodiq.workflows.base.get_config", lambda: mock_cfg)

        class SlowUntimedStep(Step):
            step_id = "slow_untimed"
            has_timeout = False

            async def exec(self) -> StepResult:
                await asyncio.sleep(0.1)  # > process_input_timeout
                return StepResult(passable=True)
        ...
        workflow = Workflow(
            api_adapter=mock_adapter,
            steps=[SlowUntimedStep, PostStep],
            fallback_step=FallbackStep,
            session_factory=MagicMock(),
            embedder=mock_embedder,
        )
        ...
        # Without the deadline reset, PostStep would receive a negative
        # remaining and CancelledError → fallback. With it, PostStep gets
        # a fresh 0.05s budget and completes cleanly.
        assert response.status_code == 200
        assert response.body == b'{"role":"assistant","content":"Done"}'
